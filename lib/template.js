...
const { loadFeatureDefinitions } = require('@lib/features.js');
const { getLegalPages } = require('@lib/legalPages');
const { getEBGOAuthAuthorizeUrl, isEBGOAuthEnabled } = require('@lib/oauth.js');
const { recordPageLoadDuration } = require('@lib/sqlite/performanceStats');
...
class ViewRenderer {
    constructor(app, viewsDir, ttl = Number(process.env.DYNAMICVIEWCACHE_TTL) || 60) {
        ...
        this.manifest = fs.readFileSync(path.join(viewsDir, 'elements/manifest.ejs'), 'utf8');
        ...
        this.html_content = {
            ...
            manifest: ejs.render(this.manifest, getContextObject()),
        };
    ...
    #getLanguageData(langCode, page) {
        ...
        if (!langData || !requiredKeys) {
            process.log.warn(`Missing language data or mapping for ${langCode}, ${page} (Page exists, but not translation config)`);
            return {};
        }

        const languageData = requiredKeys.reduce((result, key) => {
            if (langData[key]) {
                result[key] = JSON.parse(JSON.stringify(langData[key]));
            } else {
    ...
    registerStaticRoutes(sourceDir, exclude = [], customRoutes = {}, baseDir = sourceDir) {
        ...
        fs.readdirSync(sourceDir, { withFileTypes: true }).forEach(dirent => {
            ...
            const routedSourcePath = relativeSourcePath.split('/').map((segment, index, allSegments) => {
                const previousSegment = allSegments[index - 1];
                if (previousSegment && segment.startsWith(`${previousSegment}_`)) {
                    return segment.slice(previousSegment.length + 1);
                }
                return segment;
            }).join('/');
            let cacheKey = '/' + routedSourcePath.replace(extension, '');
            ...
            if (dirent.isDirectory()) {
                ...
                this.staticRouteDefinitions.set(cacheKey, { sourcePath });
                ...
                if (!this.registeredRoutes.has(cacheKey)) {
                    this.app.get(cacheKey, (req, res) => {
                        ...
                        res.once('finish', () => {
                            recordPageLoadDuration(cacheKey, performance.now() - pageLoadStart);
                        });
    ...
    /**
     * Flush all template caches and re-render all registered static pages.
     */
    flushAllCachesAndRenderStaticPages() {
        flushContextObjectCache();
        this.dynamicCache = {};
        this.static_pages = {};
        this.html_content = {
            navbar: ejs.render(this.dashboard_navbar, getContextObject()),
        ...
        this.staticRouteDefinitions.forEach(({ sourcePath }, cacheKey) => {
            Object.keys(process.availableLanguages).forEach(language => {
                const localizedCacheKey = `${cacheKey}_${language}`;
                const context = getContextObject();
                const startTime = performance.now();
                ...
                try {
                    str = ejs.render(fs.readFileSync(sourcePath, 'utf8'), { ...context, html_content: this.html_content, language: this.#getLanguageData(language, cacheKey) }, { filename: sourcePath });
                } catch (err) {
                    throw new RenderError("Static Rendering Error").setError(err);
                }
                ...
                if (process.env.MINIFY == "true") {
                    ...
                    process.log.debug(`[RENCACHE] Re-rendered static page: ${localizedCacheKey} (${Math.floor((endTime * 1000) - (startTime * 1000))}us)`);
                    this.static_pages[localizedCacheKey] = str;
                }
            });
        });

        process.log.system(`[RENCACHE] Flushed template caches and re-rendered ${this.staticRouteDefinitions.size} static pages.`);
    }
    ...
    registerRoute(filePath, routePath) {
        this.app.get(routePath, (req, res) => {
            const pageLoadStart = performance.now();
            res.once('finish', () => {
                recordPageLoadDuration(routePath, performance.now() - pageLoadStart);
            });
            ...
            const startTime = performance.now();
            ejs.renderFile(filePath, { params: req.params, html_content: this.html_content, language: this.#getLanguageData(cookie_language, routePath), ...getContextObject() }, (err, str) => {
    ...
    convertFilenameToRoute(filePath) {
        ...
        let segments = normalizedPath.split('/').map((segment, index, allSegments) => {
            const previousSegment = allSegments[index - 1];
            if (previousSegment && segment.startsWith(`${previousSegment}_`)) {
                segment = segment.slice(previousSegment.length + 1);
            }
...
const getContextObject = () => {
    if (cachedContextObject) return cachedContextObject;
    ...
    return cachedContextObject;
};

const flushContextObjectCache = () => {
    cachedContextObject = null;
};
