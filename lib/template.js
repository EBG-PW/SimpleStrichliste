...
const { loadFeatureDefinitions, getEnabledFeatures, getEnabledFeatureDefinitions } = require('@lib/features.js');
const { getFeatureSettings } = require('@lib/sqlite/settings.js');
...
class ViewRenderer {
    ...
    #getLanguageData(langCode, page) {
        ...
        const featureLangData = process.availableFeatureLanguages?.[langCode] || {};
        const featureNames = page === '/admin/settings'
            ? Object.keys(featureDefinitions)
            : Object.entries(featureSettings).filter(([, enabled]) => enabled).map(([featureName]) => featureName);

        featureNames.forEach(featureName => {
            if (!featureLangData[featureName]) return;

            requiredKeys.forEach(key => {
                if (featureLangData[featureName][key]) {
                    if (!languageData[key]) languageData[key] = {};
                    deepMerge(languageData[key], featureLangData[featureName][key]);
                }
            });
        });
...
const getContextObject = () => {
    ...
    cachedContextObject = {
        ...
        features: getEnabledFeatures(featureSettings),
        featureDefinitions: getEnabledFeatureDefinitions(featureSettings),
        featureSettingsDefinitions: featureDefinitions,
