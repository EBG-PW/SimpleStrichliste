...
const staticStoragePath = path.join(storagePath, 'static');
// Ensure the storage directory exists
fs.mkdirSync(storagePath, { recursive: true });
fs.mkdirSync(staticStoragePath, { recursive: true });

/**
 * Resolve a relative path inside a base path, ensuring it does not escape the base directory.
 * @param {String} basePath 
 * @param {String} relativePath 
 * @returns {String}
 */
const resolveInside = (basePath, relativePath) => {
...
/**
 * Get the file path of a static file, optionally falling back to the public directory.
 * @param {String} relativePath 
 * @param {Object} options 
 * @param {Boolean} options.fallbackToPublic - Whether to fallback to the public directory if the file is not found in the static storage.
 * @returns {String}
 */
const getStaticFilePath = (relativePath, { fallbackToPublic = false } = {}) => {
    const storageFilePath = resolveInside(staticStoragePath, relativePath);
    if (fs.existsSync(storageFilePath)) return storageFilePath;

    if (fallbackToPublic) {
        const publicFilePath = resolveInside(publicPath, relativePath);
        if (fs.existsSync(publicFilePath)) return publicFilePath;
    }

    throw new FilesystemError('Static file not found');
};
...
const writefavicon = async (buffer) => {
    const iconsPath = path.join(staticStoragePath, 'icons');
    const faviconPath = path.join(staticStoragePath, 'favicon.ico');
