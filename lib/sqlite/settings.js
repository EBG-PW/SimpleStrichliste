...
/**
 * Get all feature flags from app settings.
 * Settings are stored as feature_<name> and exposed as { [name]: boolean }.
 * @returns {Object}
 */
const getFeatureSettings = () => {
    const settings = db.prepare("SELECT setting_key, setting_value FROM app_settings WHERE setting_key LIKE 'feature_%'").all();
    return settings.reduce((features, setting) => {
        const featureName = setting.setting_key.replace(/^feature_/, '');
        features[featureName] = setting.setting_value === 'true';
        return features;
    }, {});
};

/**
 * Ensure every installed feature has a toggle setting.
 * @param {Object} featureDefinitions
 */
const ensureFeatureSettings = (featureDefinitions) => {
    const insertSetting = db.prepare('INSERT OR IGNORE INTO app_settings (setting_key, setting_value) VALUES (?, ?)');

    Object.entries(featureDefinitions).forEach(([featureName, definition]) => {
        insertSetting.run(`feature_${featureName}`, definition.enabledByDefault === true ? 'true' : 'false');
    });
};
