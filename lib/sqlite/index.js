...
function closeDatabases() {
    if (db) {
        try {
            ...
            process.log.system('WAL checkpoint completed successfully.');
        } catch (checkpointErr) {
            process.log.error('Warning: Could not checkpoint WAL file during shutdown:', checkpointErr);
        }
        ...
        try {
            ...
            console.log(err);
            process.log.error('Error during clean shutdown:', err);

            // Fallback attempt to force close if it stalled
