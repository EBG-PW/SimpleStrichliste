...
const getUser = async (user_id) => {
  return db.prepare('SELECT uuid, name, email, username, user_role, balance / 100.0 as balance, language FROM users where id = ?').get(user_id);
};
...
const getUserByUUID = async (uuid) => {
  return db.prepare('SELECT uuid, name, email, username, user_role, balance / 100.0 as balance, language FROM users WHERE uuid = ?').get(uuid);
};
...
const setBalance = (userUuid, balance, initiatorId) => {
  ...
  const executeUpdate = db.transaction((uuid, target) => {
    const user = db.prepare('SELECT id, balance FROM users WHERE uuid = ?').get(uuid);
...
const purchaseItem = async (userUUID, itemUUID, quantity, initiatorId) => {
  const transaction = db.transaction((userUUID, itemUUID, quantity, initiatorId) => {
    ...
    if (newBalance < 0) throw new PurchaseError(`PurchaseErrorFunds`).withStatus(400);
  ...
  if (groupbyday) {
    query = `
      ...
      WHERE
        t.user_id = ?
      GROUP BY
        DATE(t.transaction_timestamp), t.item_id, t.user_id, i.uuid, i.name, u.uuid, u.name, u.username, initiator.name, initiator.username
      ORDER BY
...
const updateUserGroupByUUID = async (user_uuid, new_user_group) => {
  db.prepare('UPDATE users SET user_role = ? WHERE uuid = ?').run(new_user_group, user_uuid);
};
