...
const staticImageCheck = Joi.object({
    filename: Joi.string().valid('low-funds').required(),
});

router.get('/static/:filename', limiter(1), async (req, res) => {
    const { filename } = await staticImageCheck.validateAsync(req.params);
    try {
        const stream = await readImage('static', filename, 'webp', true);
